...
interface IUseRandomDataColor {
  createRandomColor: (
    condition: RGBBooleanFunction
  ) => [number, number, number];
  rgbToString: (r: number, g: number, b: number, a?: number) => string;
  convertRGBToHSL: (
    r: number,
    g: number,
    b: number
...
export default function useRandomDataColor(): IUseRandomDataColor {
  const { imgData } = useMainContext();
  ...
  const createRandomColor = useCallback(
    (
      condition: RGBBooleanFunction = (): boolean => false
    ): [number, number, number] => {
  ...
  const rgbToString = useCallback(
    (r: number, g: number, b: number, a = 1): string => {
      return `rgb(${r},${g},${b},${a})`;
    },
    []
  );
  ...
  const convertRGBToHSL = useCallback(
